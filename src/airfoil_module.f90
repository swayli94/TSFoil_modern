...
contains
  ...
  subroutine PRBODY()
    ...
    if (.not. PHYS) then
      ...
      write(UNIT_OUTPUT, '(1x,"AIRFOIL VOLUME =",F12.8,32X,"MAX CAMBER    =",F10.6//20X,&
            &"UPPER  SURFACE",14X,"LOWER  SURFACE")') VOL, CAMAX
      write(UNIT_OUTPUT, '(8X,"X",1X,2(12X,"F",9X,"DF/DX"),8X,"THICKNESS",4X,"CAMBER")')

      do II = 1, NFOIL
        write(UNIT_OUTPUT, '(1X,F12.8,2X,2F12.8,2(3X,2F12.8))') &
            &XFOIL(II), FU(II), FXU(II), FL(II), FXL(II), THICK(II), CAMBER(II)
      end do
      ...
      CAMAX = DELTA*CAMAX
      VOLUME = VOL*DELTA
      write(UNIT_OUTPUT, '(1x,A,3X,A,F10.6)') &
            &"PRINTOUT IN PHYSICAL VARIABLES NORMALIZED BY CHORD LENGTH","MAX THICKNESS =", THMAX
      write(UNIT_OUTPUT, '(1x,"AIRFOIL VOLUME =",F12.8,32X,"MAX CAMBER    =",F10.6//20X,&
            &"UPPER  SURFACE",14X,"LOWER  SURFACE")') VOLUME, CAMAX
